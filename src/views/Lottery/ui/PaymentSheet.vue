...
import type { Prize, SparklePosition } from '../model/types'
...
const quantityPercentage = computed(() => {
  return ((props.quantity - props.minQuantity) / (props.maxQuantity - props.minQuantity)) * 100
})
      ...
      <div class="relative pt-12 pb-4">
        <div class="relative">
          <div
            ...
            :style="{
              background: `linear-gradient(to right, #A79EFF 0%, #A79EFF ${quantityPercentage}%, #F5F0FF ${quantityPercentage}%, #F5F0FF 100%)`,
            }"
          ></div>
          <input
            :value="quantity"
